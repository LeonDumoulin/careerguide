<template>
  <div>
    <table id="faculity_table" class="display mt-5" style="width: 100%">
      <thead>
        <tr>
          <th>اسم الكلية</th>
          <th>وصف الكلية</th>
...
$("body").on("click", ".delete_button", function () {
  ...
  $(".confirm-delete").click(function () {
    axios
      .post(
        "https://roadmapapi.ahmedhesham.systems/api/v1/faculty/delete",
        {id:select.id},
        {
          headers: {
    ...
    $("body").on("click", ".edit_button", function () {
        ...
        $("#editfaculity").on("show.bs.modal", function(e) {
          ...
          var hamada = $(e.relatedTarget);
            ...
            var modalVariable = {
              faculity_nameValue: fromTableToModal.row.find(".faculity_name").text(),
              faculity_descreptionValue: fromTableToModal.row.find(".faculity_descreption").text(),
              faculity_imgValue: fromTableToModal.row.find(".faculity_img > img").attr('src'),
              faculity_idValue: fromTableToModal.row.find(".hide_column > span").text(),
            }
            $(this).find(".faculity_name").val(modalVariable.faculity_nameValue); // from table to model
            $(this).find(".faculity_descreption").val(modalVariable.faculity_descreptionValue); // from table to model
            $(this).find(".faculity_img").attr('src' , modalVariable.faculity_imgValue); // from table to model
            $(this).find(".faculity_id").val(modalVariable.faculity_idValue); // from table to model
         /*  */
        });
    });
...
export default {
  ...
  methods: {
    onScriptLoaded() {
      ...
      this.dataTable = $("#faculity_table").DataTable({
        language: {
        ...
        ajax: "https://roadmapapi.ahmedhesham.systems/api/v1/faculty/index",
        columns: [
          { data: "name",
          className: "faculity_name" },
          { data: "des",
          className: "faculity_descreption" },
          {
            data: "image",

          className: "faculity_img",
  ...
  "class="edit_button" data-toggle="modal" data-target="#editfaculity">
      تعديل
    </button>`,
          },
          {
