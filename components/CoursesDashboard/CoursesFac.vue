<template>
  <div>
    <table id="courses_table" class="display mt-5" style="width: 100%">
      <thead>
        <tr>
          <th>اسم الكورس</th>
          <th>وصف الكورس</th>
          <th>صورة الكورس</th>
          <th>معرف التكنولوجي</th>
          <th>تابع التكنولوجي</th>
          <th>حذف</th>
          <th>تعديل</th>
          <th>id</th>
        </tr>
...
$("body").on("click", ".delete_button", function () {
  ...
  $(".confirm-delete").click(function () {
    axios
      .post(
        "https://roadmapapi.ahmedhesham.systems/api/v1/courses/delete",
        {id:select.id},
        {
          headers: {
    ...
    $("body").on("click", ".edit_button", function () {
        ...
        $("#editcourses").on("show.bs.modal", function(e) {
          ...
          var hamada = $(e.relatedTarget);
            ...
            var modalVariable = {
              //technological_id
              courses_nameValue: fromTableToModal.row.find(".courses_name").text(),
              courses_descreptionValue: fromTableToModal.row.find(".courses_descreption").text(),
              courses_fidValue: fromTableToModal.row.find(".technological_id").text(),
              courses_imgValue: fromTableToModal.row.find(".courses_img > img").attr('src'),
              course_idValue: fromTableToModal.row.find(".hide_column > span").text(),
            }
            $(this).find(".courses_name").val(modalVariable.courses_nameValue);
            $(this).find(".courses_descreption").val(modalVariable.courses_descreptionValue);
            $(this).find(".courses_img").attr('src' , modalVariable.courses_imgValue);
            $(this).find(".courses_id").val(modalVariable.course_idValue);
            $(this).find(".technological_id").val(modalVariable.courses_fidValue);
...
export default {
  ...
  methods: {
    onScriptLoaded() {
      ...
      this.dataTable = $("#courses_table").DataTable({
        language: {
        ...
        ajax: "https://roadmapapi.ahmedhesham.systems/api/v1/courses/index",
        columns: [
          { data: "name",
          className: "courses_name" },
          { data: "des",
          className: "courses_descreption" },
          {
            data: "image",
          ...
          { data: "technological_id",
          className: "technological_id d-none" },
          { data: "technological.name"},
          //
          {
            data: null,
            className: "center",
  ...
  "class="edit_button" data-toggle="modal" data-target="#editcourses">
      تعديل
    </button>`,
          },
          {
